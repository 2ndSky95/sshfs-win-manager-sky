export default {
  app: {
    name: 'SSHFS-Win Manager Sky',
    serviceActive: 'Service actif',
    serviceActiveShort: 'Actif'
  },
  nav: {
    expand: 'Déplier le menu',
    collapse: 'Replier le menu',
    connections: 'Connexions',
    favorites: 'Favoris',
    settings: 'Paramètres',
    about: 'À propos'
  },
  common: {
    cancel: 'Annuler',
    save: 'Enregistrer',
    ok: 'OK',
    loading: 'Chargement...',
    connect: 'Connecter',
    connecting: 'Connexion...',
    disconnect: 'Déconnecter',
    edit: 'Éditer',
    duplicate: 'Dupliquer',
    delete: 'Supprimer',
    copy: 'Copier',
    openFolder: 'Ouvrir le dossier',
    openTerminal: 'Ouvrir dans le terminal',
    close: 'Fermer'
  },
  status: {
    connected: 'Connectée',
    disconnected: 'Déconnectée',
    connecting: 'Connexion en cours',
    disconnecting: 'Déconnexion en cours',
    unknown: 'Inconnu'
  },
  list: {
    searchPlaceholder: 'Rechercher une connexion...',
    sortName: 'A-Z',
    sortStatus: 'Statut',
    sortManual: 'Manuel',
    editOrder: 'Édition',
    editDone: 'Terminé',
    emptyTitle: 'Aucune connexion',
    emptyText: 'Ajoute une première connexion pour commencer.',
    moveUp: 'Monter',
    moveDown: 'Descendre',
    details: 'Détails',
    addFavorite: 'Ajouter aux favoris',
    removeFavorite: 'Retirer des favoris'
  },
  detail: {
    newConnection: 'Nouvelle connexion',
    selectTitle: 'Sélectionne une connexion',
    selectText: 'Les informations et actions rapides apparaîtront ici.',
    changeIcon: 'Changer le logo',
    removeIcon: 'Retirer le logo',
    status: 'Statut',
    connectionType: 'Type de connexion',
    address: 'Adresse',
    port: 'Port',
    mountPoint: 'Point de montage',
    remotePath: 'Chemin distant',
    user: 'Utilisateur',
    sshCommand: 'Connexion SSH',
    copySshCommand: 'Copier la commande SSH',
    actions: 'Actions',
    expandPanel: 'Déplier le panneau de détail',
    collapsePanel: 'Replier le panneau de détail',
    connectionsCount: '{count} connexions',
    connectedCount: '{count} connectées',
    busyCount: '{count} en cours',
    debugOutput: 'Debug output',
    clearDebug: 'Vider les logs',
    copyDebug: 'Copier les logs'
  },
  settings: {
    title: 'Paramètres',
    subtitle: 'Configuration générale, thème et import/export des connexions.',
    sectionGeneral: 'Général',
    sectionPasskey: 'Passkey',
    sectionData: 'Sauvegarde & import des connexions',
    sshfsBinary: 'Binaire SSHFS',
    processTimeout: 'Timeout de connexion (sec)',
    theme: 'Thème',
    windowOpacity: 'Transparence',
    language: 'Langue',
    languageFr: 'Français',
    languageEn: 'English',
    languageDe: 'Deutsch',
    languageEs: 'Español',
    languageIt: 'Italiano',
    languageZh: '中文',
    passkeyEnabled: 'Passkey (chiffrer les mots de passe)',
    passkeyDisableTitle: 'Désactiver le passkey ?',
    passkeyDisableConfirm: 'Attention : les mots de passe enregistrés seront stockés en CLAIR dans le fichier de configuration. Toute personne ayant accès à vos fichiers pourra les lire.\n\nLe passkey sera demandé une fois pour déchiffrer les mots de passe existants.',
    passkeyRetention: 'Rétention passkey',
    passkeyAlways: 'Toujours demander',
    passkey1h: '1 heure',
    passkey12h: '12 heures',
    passkey1d: '1 jour',
    passkey2d: '2 jours',
    startupWithOS: 'Démarrer avec {os}',
    displayTrayMessageOnClose: 'Message tray à la fermeture',
    closeWindowQuits: 'Fermer quitte l’application',
    startInTray: 'Démarrer dans la zone de notification',
    startInMenuBar: 'Démarrer dans la barre de menus',
    blurAddresses: 'Flouter les adresses',
    showDebugPanel: 'Afficher le panneau debug',
    compactMode: 'Mode compact',
    exportJson: 'Export JSON',
    importJson: 'Import JSON',
    importLegacy: 'Importer ancienne configuration',
    connections: 'Connexions',
    timeInSeconds: 'Temps en secondes',
    importConfirm: 'Importer {count} connexion(s) ? Cela remplacera la liste actuelle.',
    importEncryptPrompt: 'Les connexions importées contiennent des mots de passe en clair.\n\nLes chiffrer maintenant avec le passkey ?\nOui = saisir ou définir le passkey. Non = ils restent en clair et se connectent sans passkey.',
    connectEncryptPrompt: 'Le mot de passe de cette connexion est stocké en clair alors que le passkey est activé.\n\nLe chiffrer maintenant avec le passkey ?\nOui = saisir ou définir le passkey. Non = se connecter sans passkey.',
    exportFailed: 'Export impossible. Réessaie.',
    invalidImportJson: 'Fichier d’import invalide : le JSON ne peut pas être lu.',
    invalidImportConnections: 'Fichier d’import invalide : liste des connexions introuvable.',
    legacyImportNotFound: 'Aucune ancienne configuration SSHFS-Win Manager n’a été trouvée.',
    legacyImportReadFailed: 'Impossible de lire l’ancienne configuration : {error}',
    legacyImportEmpty: 'Ancienne configuration trouvée, mais aucune connexion n’a été détectée.',
    legacyImportConfirm: '{count} connexion(s) trouvée(s) dans l’ancienne configuration.\n\nFichier : {filePath}\n\nImporter et fusionner avec les connexions actuelles ?',
    legacyImportConfirmWithPasswords: '{count} connexion(s) trouvée(s) dans l’ancienne configuration.\n\nFichier : {filePath}\n\nAttention : certaines connexions contiennent un mot de passe stocké dans l’ancien fichier. Ils seront importés tels quels.\n\nImporter et fusionner avec les connexions actuelles ?'
  },
  about: {
    versionLine: 'Version {version} - Sky Edition.',
    maintainedBy: 'Sky Edition — une version personnalisée de SSHFS-Win Manager Evo par Fabrice Simonet.',
    website: 'Site web :',
    basedOn: 'Basé sur SSHFS-Win Manager Evo (Fabrice Simonet) et le projet original SSHFS-Win Manager créé par Evandro Araujo.',
    license: 'Licence MIT. Les notices de copyright et de licence des projets originaux sont conservées.',
    libraries: 'Bibliothèques principales'
  },
  connectionForm: {
    addTitle: 'Ajouter une connexion',
    editTitle: 'Modifier une connexion',
    basic: 'BASIQUE',
    advanced: 'AVANCÉ',
    name: 'Nom',
    namePlaceholder: 'ex. Mon serveur de développement 1',
    connection: 'Connexion',
    host: 'IP/Hôte',
    hostPlaceholder: 'ex. 127.0.0.1 ou mon.domaine.com',
    portPlaceholder: 'ex. 22',
    user: 'Utilisateur',
    userPlaceholder: 'ex. root',
    authMethod: 'Méthode d’authentification',
    password: 'Mot de passe',
    keyFile: 'Fichier de clé',
    keyFilePlaceholder: 'ex. C:\\Users\\moi\\.ssh\\id_rsa',
    selectPrivateKey: 'Sélectionner la clé privée',
    key: 'Clé',
    keyPlaceholder: 'ex. ssh-rsa AAAAB3Nz...',
    remote: 'Distant',
    path: 'Chemin',
    pathPlaceholder: 'ex. /home/john',
    local: 'Local',
    driveLetter: 'Lettre de lecteur',
    mountPath: 'Chemin de montage',
    mountPathPlaceholder: 'ex. /home/moi/montages/serveur',
    selectMountPath: 'Sélectionner le dossier de montage',
    autoMountPath: 'Vide = auto : {path}',
    autoDrive: 'Auto (prochaine lettre libre)',
    connectOnStartup: 'Connexion au démarrage',
    reconnect: 'Reconnexion si la connexion est perdue',
    customOptions: 'Paramètres de ligne de commande personnalisés',
    paramValue: 'Valeur du paramètre',
    noParams: 'Aucun paramètre ajouté',
    addParam: 'Ajouter un paramètre',
    selectOption: '(Sélectionne une option)'
  },
  auth: {
    keyFile: 'Clé privée',
    keyFilePassphrase: 'Clé privée + passphrase',
    keyFileInteractive: 'Clé privée + PAM/OTP',
    keyFilePassphraseInteractive: 'Clé privée + passphrase + PAM/OTP',
    password: 'Mot de passe',
    passwordAsk: 'Mot de passe (demander à la connexion)',
    interactive: 'PAM/OTP uniquement (sans clé) [BETA]',
    interactiveHint: 'Mode BETA pour les authentifications clavier-interactives sans clé privée (PAM, OTP, SSO challenge). Le système demandera une information de validation au moment de la connexion.',
    keyPassphraseHint: 'La passphrase de la clé privée sera demandée au démarrage de la connexion. La passphrase n’est pas sauvegardée dans la configuration.',
    keyInteractiveHint: 'Pour les serveurs qui demandent une clé privée puis un challenge PAM/OTP. Le code de vérification et le mot de passe seront demandés à la connexion.',
    keyPassphraseInteractiveHint: 'Pour les clés privées protégées par passphrase suivies d’un challenge PAM/OTP. La passphrase, le code de vérification et le mot de passe seront demandés à la connexion.'
  },
  passwordPrompt: {
    keyPassphrase: 'Passphrase de la clé',
    verificationCode: 'Code de vérification',
    password: 'Mot de passe',
    passwordOptional: 'Mot de passe (optionnel)',
    passphrase: 'Passphrase',
    pamTitle: "PAM/OTP pour {user}{'@'}{host}",
    title: "{label} pour {user}{'@'}{host}"
  },
  passkeyPrompt: {
    createTitle: 'Créer la passkey globale',
    unlockTitle: 'Déverrouiller les secrets',
    createDescription: 'Cette passkey chiffre les mots de passe stockés dans le JSON. Elle ne sera pas enregistrée. Note-la bien : sans elle, les secrets chiffrés seront irrécupérables.',
    unlockDescription: 'Saisis la passkey globale pour déchiffrer temporairement les mots de passe nécessaires à la connexion.',
    passkey: 'Passkey',
    confirmPasskey: 'Confirmer la passkey',
    emptyError: 'La passkey ne peut pas être vide.',
    mismatchError: 'Les deux passkeys ne correspondent pas.'
  },
  notifications: {
    cannotConnect: 'Impossible de se connecter à "{name}" : {error}',
    passkeyInvalid: 'Passkey invalide : impossible de déchiffrer le mot de passe.',
    passkeyUnlocked: 'Passkey validée : secrets déverrouillés.',
    trayStillRunning: 'Le programme continue de tourner dans la zone de notification',
    debugCopied: 'Logs copiés dans le presse-papiers',
    sshCommandCopied: 'Commande SSH copiée dans le presse-papiers',
    terminalPasswordCopied: 'Mot de passe copié dans le presse-papiers',
    terminalOpenFailed: 'Impossible d’ouvrir le terminal : {error}',
    disconnectedError: '"{name}" a été déconnectée à cause d’une erreur de connexion.\nVérifie ta connexion internet.',
    trackedAlternative: '"{name}" est suivie avec la méthode alternative',
    processTimeout: 'Timeout du process : impossible de se connecter à "{name}"',
    legacyImportDone: '{count} connexion(s) importée(s) depuis l’ancienne configuration'
  }
}
